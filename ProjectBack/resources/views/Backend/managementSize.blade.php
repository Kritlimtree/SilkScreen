							...
							<section id="banner">
								...
								<div class="Center">
									...
									<div class="modal-dialog">
										...
										<form action="{{ route('managementSize_update') }}" method="POST" enctype="multipart/form-data">
										@csrf
										@method('PUT')
										<label for="size">ไซส์เสื้อ:</label>
									...
									<div class="modal-dialog">
										...
										<form action="{{route('managementSize_delete')}}" method="POST"  >
											@csrf
											@method('DELETE')
										<div class="modal-body">
											
											<p>Are You Sure ?</p>
											<input type="hidden" id="delete_id" name="id_shirtsize" value="DELETE">
										</div>
										<div class="footer">
										<button type="button" class=" danger" data-bs-dismiss="modal">Close</button>
										<button type="submit" class="secondary" data-loading-text="Loading...">
			...
			<script>
				...
				$(document).on('click','.editbtn',function (){
					...
					$.ajax({
						...
						method: "GET", 
						url: "/managementSize-edit/"+id,
						
						success: function (response){	
							  
							 $('#edit_id').val(id);
							 $('#shirtsize_size_edit').val(response.shirtsize[0].shirtsize_size);
							 $('#shirtsize_chest_edit').val(response.shirtsize[0].shirtsize_chest);
							 $('#shirtsize_long_edit').val(response.shirtsize[0].shirtsize_long);
							 $('#shirtsize_price_edit').val(response.shirtsize[0].shirtsize_price);
